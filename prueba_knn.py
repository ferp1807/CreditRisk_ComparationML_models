#El codigo evalua el modelo KNN utilizando 5 metricas de clasificacion y validacion cruzada
...
cv_results=cross_validate(knn_model,X_train,y_train,cv=cv,scoring=scoring,n_jobs=-1)
...
plt.title("Matriz de confusión normalizada - KNN (test 30%)")
plt.tight_layout()
plt.savefig("knn_confusion_matrix_normalized.png",dpi=200)
plt.show()
...
plt.title("Curva ROC - KNN (test 30%)")
plt.legend()
plt.tight_layout()
plt.savefig("knn_roc_curve.png",dpi=200)
plt.show()
