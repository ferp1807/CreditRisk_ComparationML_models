#El codigo evalua el modelo Arbol de Decision utilizando 5 metricas de clasificacion y validacion cruzada
...
from sklearn.preprocessing import OneHotEncoder #preprocesamiento
...
#pipeline numerico (en arbol no es necesario escalar, solo imputar)
num_proc=Pipeline([
...
#modelo arbol de decision (se dejan parametros basicos para no sobrecomplicar)
tree_model=Pipeline([
...
cv_results=cross_validate(tree_model,X_train,y_train,cv=cv,scoring=scoring,n_jobs=-1)
...
#para ROC-AUC se necesitan probabilidades (clase 1=riesgoso)
...
plt.title("Matriz de confusión normalizada - Árbol de decisión (test 30%)")
plt.tight_layout()
plt.savefig("tree_confusion_matrix_normalized.png",dpi=200)
plt.show()
...
plt.title("Curva ROC - Árbol de decisión (test 30%)")
plt.legend()
plt.tight_layout()
plt.savefig("tree_roc_curve.png",dpi=200)
plt.show()
